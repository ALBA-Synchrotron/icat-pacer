...
class PACERValidator(Validator):
    ...
    def _validate_check_datacite_integration_settings(self, check, field, _) -> None:
        ...
        if datacite.get("enabled") is True:
            ...
            if not language or not catalogue_doi_base_url or not publisher or not prefix or not session_suffix or not username or not password or not apiUrl or not rights_name or not rights_scheme_uri or not rights_uri or not rights_identifier_scheme or not rights_identifier or not funder_name or not funder_identifier or not funder_identifier_type:
                self._error(field,
                            "'language', 'dataCatalogueDoiBaseUrl', 'publisher', 'prefix', 'sessionSuffix', 'username', 'password', 'apiUrl', 'rightsName', 'rightsSchemeUri', 'rightsUri', 'rightsIdentifierScheme', 'rightsIdentifier', 'funderName', 'funderIdentifier', 'funderIdentifierType' are required when 'integrations.datacite.enabled' is True")
